  ...
  try {
    const stockData = await Promise.all(
      tickersArr.map(async (ticker) => {
        const url = `https://polygon-api-worker.ajkendal-openai.workers.dev/?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`;
  ...
  const messages = [
    {
      role: 'system',
      content:
        'I need a shortreport no more than 150 words on the following data for stock predictions, short and easy for a high-school student to understand if to buy or sell. Use Examples provided between ### to set the style and tone of your response.',
    },
    {
      role: 'user',
      content: `${data}
